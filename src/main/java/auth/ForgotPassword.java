...
public void requestForgotPassword() {
    ...
    try {
        ...
        JSONArray respBranch = parseJsonArrayResponse(connBranch);
        if (respBranch != null) {
            String[] items = new String[respBranch.length() + 1];
            items[0] = "Select branch";
            for (int i = 0; i < respBranch.length(); i++) {
                JSONObject o = respBranch.getJSONObject(i);
                items[i + 1] = o.getString("branchid") + " (" + o.getString("name") + ")";
            }
            branchCombo.setModel(new DefaultComboBoxModel<>(items));
        }
    } catch (Exception ex) {}
    ...
    findButton.addActionListener(e -> {
        ...
        try {
            ...
            if (resp.getBoolean("response")) {
                ...
                JOptionPane.showMessageDialog(dialog, resp.optString("error", "Error"), "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {}
    });
