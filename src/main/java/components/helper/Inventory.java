...
public class Inventory extends ServerURL implements CustomBorder {
    ...
    public void loadInventoryRequests(DefaultTableModel model) {
        ...
        for (int i = 0; i < arr.length(); i++) {
            ...
            String last_restock = obj.getString("last_restock");

            if (!branch.equals(user.getString("branch")) && !user.getString("branch").equals("all")) {
                continue;
            }

            String key = generateKey(item_name, branch);
            incomingKeys.add(key);

            boolean found = false;
            for (int j = 0; j < model.getRowCount(); j++) {
                String existingItem = model.getValueAt(j, 0).toString();
                String existingBranch = model.getValueAt(j, 3).toString();
                String existingKey = generateKey(existingItem, existingBranch);

                if (existingKey.equals(key)) {
                    model.setValueAt(quantity, j, 1);
                    model.setValueAt(threshold, j, 2);
                    model.setValueAt(status, j, 4);
                    model.setValueAt(last_restock, j, 5);
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                model.addRow(new Object[]{ item_name, quantity, threshold, branch, status, last_restock });
            }
        ...
        for (int i = model.getRowCount() - 1; i >= 0; i--) {
            ...
            String existingBranch = model.getValueAt(i, 3).toString();
            String existingKey = generateKey(existingItem, existingBranch);
    ...
    public void newInventory(DefaultTableModel tableModel) {
        ...
        thresholdSpinner.addChangeListener(e -> {
            ...
            quantityModel.setMaximum(newThreshold); // enforce max

            if (currentQty > newThreshold) {
                quantitySpinner.setValue(newThreshold); // fix if exceeded
            }
        });
